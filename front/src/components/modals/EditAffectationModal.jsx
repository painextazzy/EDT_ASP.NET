...
const EditEventModal = ({ 
  ...
  const checkConflicts = async () => {
    ...
    let sallesIds = [];
    if (editingEvent.type === 'Examen' || editingEvent.type === 'Présentation') {
      sallesIds = editingEvent.salles || [];
    } else {
      sallesIds = editingEvent.salle ? [editingEvent.salle] : [];
    }
    ...
    try {
      ...
      for (const salleId of sallesIds) {
        try {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const data = {
        idEnseignement: editingEvent.idEnseignement || editingEvent.enseignementId,
        typeEvenement: editingEvent.type,
        dateDebut: dateStart.toISOString(),
        dateFin: dateEnd.toISOString(),
        idSalles: sallesIds,
        motifAnnulation: editingEvent.motifAnnulation || null
      };
              ...
              <div className="flex items-start gap-4 mb-4">
                ...
                <div>
                  ...
                  <div className="mt-3 p-3 bg-gray-50 rounded-lg border border-gray-200">
                    ...
                    <p className="text-xs text-gray-500">
                      <span className="font-semibold">Salle(s):</span> {editingEvent.type === 'Examen' || editingEvent.type === 'Présentation' 
        ...
        <form className="p-8 space-y-6" onSubmit={(e) => { e.preventDefault(); handleSubmit(); }}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              ...
              <select 
                value={editingEvent.title || editingEvent.titre || ''}
                onChange={(e) => {
                  setEditingEvent({ ...editingEvent, title: e.target.value, titre: e.target.value });
                  setConflictError('');
                  setConflictWarning('');
                }}
                className="w-full border border-gray-300 rounded-eight focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-gray-700 text-sm py-2.5 px-3 bg-white"
              >
                ...
                {coursOptions && coursOptions.map(c => (
                  <option key={c.id} value={c.nom || c.name}>{c.nom || c.name}</option>
                ))}
              </select>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  {heuresOptions && heuresOptions.map(h => <option key={h} value={h}>{h}</option>)}
                </select>
          ...
          <div className="space-y-2">
            ...
            {(editingEvent.type === 'Examen' || editingEvent.type === 'Présentation') ? (
              <div>
                <select 
                  value={editingEvent.salles?.[editingEvent.salles.length - 1] || ''}
                  onChange={(e) => {
                    if (e.target.value && !editingEvent.salles?.includes(parseInt(e.target.value))) {
                      setEditingEvent({ 
                        ...editingEvent, 
                ...
                >
                  ...
                  {sallesOptions && sallesOptions
                    ...
                    .map(s => <option key={s.id} value={s.id}>{s.numero || s.nom}</option>)}
                </select>
                    ...
                    {editingEvent.salles.map((salleId, idx) => {
                      ...
                      return (
                        <span key={idx} className="inline-flex items-center gap-1 px-3 py-1.5 bg-blue-50 text-blue-700 rounded-lg text-sm border border-blue-200">
                          {salle?.numero || salle?.nom || `Salle ${salleId}`}
                          <button
                            type="button"
              ...
              >
                ...
                {sallesOptions && sallesOptions.map(s => <option key={s.id} value={s.id}>{s.numero || s.nom}</option>)}
              </select>
            )}
          </div>
        </form>
