  ...
  batiments: [
    {
      ...
      salles: [
        ...
        },
        {
          ...
          numero: "A-004",
          statut: "LIBRE",
          courActuel: "Soutenance de Stage",
...
function AddRoomModal({ onClose, onAdd, batiments }) {
  ...
  const handleAdd = () => {
    ...
    onAdd({ numero: numero.trim(), batiment, etage, parcours, mention });
    onClose();
  };
          ...
          <div style={{ position: "relative", marginBottom: 14 }}>
            <select
              ...
              style={{
                width: "100%", padding: "11px 36px 11px 14px",
                borderRadius: 10, border: "1.5px solid #e2e8f0",
                fontSize: 14, color: parcours ? "#0f172a" : "#94a3b8",
                outline: "none", appearance: "none", cursor: "pointer",
                fontFamily: "'DM Sans', sans-serif", background: "#fff",
                boxSizing: "border-box",
              }}
        ...
        <div style={{
          ...
          <button
            ...
            style={{
              ...
              background: "transparent", fontSize: 14, color: "#64748b",
          ...
          <button
            ...
            style={{
              ...
              background: numero && batiment ? "#10b981" : "#94a3b8",
              border: "none", fontSize: 14, fontWeight: 700,
              color: "#fff", cursor: numero && batiment ? "pointer" : "not-allowed",
              fontFamily: "'DM Sans', sans-serif",
              transition: "background 0.2s, transform 0.1s",
              transform: "scale(1)",
            }}
            onMouseEnter={e => { if (numero && batiment) e.target.style.background = "#059669"; }}
            onMouseLeave={e => { if (numero && batiment) e.target.style.background = "#10b981"; }}
          >
            Ajouter
          </button>
        </div>
...
// Settings Modal
function SettingsModal({ salle, batimentLabel, onClose, onDelete, onToggle }) {
  return (
    <div style={{
      ...
      <div style={{
        background: "#fff", borderRadius: 18, width: 320,
        ...
        <div style={{ padding: "22px 24px 18px", borderBottom: "1px solid #f1f5f9", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            <div style={{ fontSize: 11, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.08em" }}>Salle</div>
            <div style={{ fontSize: 20, fontWeight: 800, color: "#0f172a", fontFamily: "'DM Sans', sans-serif" }}>{salle.numero}</div>
            <div style={{ fontSize: 12, color: "#64748b" }}>{batimentLabel}</div>
          </div>
          <button onClick={onClose} style={{ background: "none", border: "none", cursor: "pointer", color: "#94a3b8" }}><XIcon /></button>
        </div>
        <div style={{ padding: "16px 24px 24px" }}>
          <button
            onClick={() => { onToggle(salle.id); onClose(); }}
            style={{
          ...
          >
            {salle.statut === "LIBRE" ? "🔴  Marquer comme occupée" : "🟢  Marquer comme libre"}
          </button>
          <button
            onClick={() => { onDelete(salle.id); onClose(); }}
            style={{
              width: "100%", padding: "12px 16px", borderRadius: 12,
...
// Salle Card
function SalleCard({ salle, batimentId, onSettings }) {
    ...
    >
      ...
      <div style={{ padding: "14px 16px" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <button
            onClick={() => onSettings(salle, batimentId)}
            style={{
  ...
  return (
    <div
      ...
      style={{
        ...
        minHeight: 180, cursor: "pointer", minWidth: 240,
...
export default function GestionSalles() {
  ...
  const [addToBatiment, setAddToBatiment] = useState(null);
  const [settingsSalle, setSettingsSalle] = useState(null);
  const [settingsBatiment, setSettingsBatiment] = useState(null);

  const handleAddRoom = ({ numero, batiment, etage, parcours, mention }) => {
    const newSalle = {
      id: `salle-${Date.now()}`,
      numero,
      statut: "LIBRE",
  ...
  const handleToggleStatut = (salleId) => {
    setData(prev => ({
      ...prev,
      batiments: prev.batiments.map(b => ({
        ...b,
        salles: b.salles.map(s =>
          s.id === salleId ? { ...s, statut: s.statut === "LIBRE" ? "OCCUPÉ" : "LIBRE" } : s
        ),
      })),
    }));
  };
    ...
    <div style={{
      ...
      {filtered.map(batiment => (
        <div key={batiment.id} style={{ marginBottom: 40 }}>
          ...
          <div style={{
            ...
            <AddRoomCard onClick={() => { setAddToBatiment(batiment.id); setShowAddModal(true); }} />
            {batiment.salles.map(salle => (
              <SalleCard
                key={salle.id}
                salle={salle}
                batimentId={batiment.id}
                onSettings={(s, bId) => { setSettingsSalle(s); setSettingsBatiment(bId); }}
              />
            ))}
          </div>
        </div>
      ...
      {showAddModal && (
        <AddRoomModal
          ...
          onClose={() => { setShowAddModal(false); setAddToBatiment(null); }}
          onAdd={handleAddRoom}
        />
      )}
      ...
      {settingsSalle && (
        <SettingsModal
          salle={settingsSalle}
          batimentLabel={data.batiments.find(b => b.id === settingsBatiment)?.label || ""}
          onClose={() => { setSettingsSalle(null); setSettingsBatiment(null); }}
          onDelete={handleDeleteRoom}
          onToggle={handleToggleStatut}
        />
      )}
    </div>
  );
